...
$sql = $mysqli->query("SELECT * FROM `request_tbl`");
    ...
    if($_SERVER['REQUEST_METHOD'] == "POST"){
        ...
        }elseif (isset($_POST['sub_admin'])) {
             
            $ADMIN_NAME = $mysqli->escape_string($_POST['new-admin-name']);
            $ADMIN_EMAIL = $mysqli->escape_string($_POST['new-admin-email']);
            $PASSWORD = $mysqli->escape_string($_POST['new-admin-password']);
            
            // Check if the user with that email or phone already exists
            $result = $mysqli->query("SELECT * FROM `users` WHERE email='$ADMIN_EMAIL'") or die($mysqli->error);
            
            // Check if the rows returned are more than 0
            if ($result->num_rows > 0) {
                echo '<script>alert("User Already Exist!");</script>';
            } else {
                $insertQuery = "INSERT INTO `users`(`name`, `email`, `password`) VALUES('$ADMIN_NAME', '$ADMIN_EMAIL', '$PASSWORD')";
