...
// $searchSql = "SELECT request_tbl.request_id, request_tbl.name, department_tbl.department_name, equipment_tbl.equipment_name, request_tbl.use_date, status.status_name, request_date 
//               FROM request_tbl 
...
//               WHERE CONCAT(request_tbl.name, equipment_tbl.equipment_name, request_tbl.use_date, request_tbl.request_date, department_tbl.department_name, status.status_name) LIKE '%$search%' AND status.status_name = 'Returned'
...
// $totalSql = "SELECT COUNT(*) FROM request_tbl 
//              INNER JOIN equipment_tbl ON request_tbl.equipment_id = equipment_tbl.equipment_id 
//              INNER JOIN department_tbl ON request_tbl.department_id = department_tbl.department_id 
//              INNER JOIN status ON request_tbl.status_id = status.status_id 
//              WHERE CONCAT(request_tbl.name, equipment_tbl.equipment_name, request_tbl.use_date, request_tbl.request_date, department_tbl.department_name, status.status_name) LIKE '%$search%' AND status.status_name = 'Returned'";
...
//                             <td><button class='status-btn bx bx' style='background-color: #FF0000; border-radius: 8px; margin-left: 5px;'>{$row['status_name']}</button></td>
//                             <td>
//                               <a href='edit_detail_report.php?det_repo_id={$row['request_id']}'><button class='status-btn bx bx bx-pencil' style='background-color: #007bff; color: white; border-radius: 8px; margin-left: 5px;'><i class='fa fa-pencil' aria-hidden='true'></i></button></a>
//                             </td>
//                           </tr>";
//                 }
//                 ?>
...
AND status.status_name = 'Returned'
...
<tbody>
    ...
    while ($row = $result->fetch_assoc()) {
        echo "<tr>
                ...
                <td>{$row['use_date']}</td>
                <td>{$row['request_date']}</td>
                <td><button class='status-btn bx bx' #0B6E4F style='background-color: #Ffa500; border-radius: 8px; margin-left: 5px;'>{$row['status_name']}</button></td>
                <td>
                  <a href='edit_detail_report.php?det_repo_id={$row['request_id']}'><button class='status-btn bx bx bx-pen' style='background-color: #007bff; color: white; border-radius: 8px; margin-left: 5px;'><i class='fa fa-pen' aria-hidden='true'></i></button></a>
                </td>
              </tr>";
    }
    ?>
